...
//        cell.lblName.text = eachPet.Name
//        cell.lblTitle.text = eachPet.Detail
    ...
    @IBAction func backToPetViewController(segue: UIStoryboardSegue) {
    }
    
    @IBAction func savePetDetail(segue: UIStoryboardSegue) {
        
        if let addTableViewController = segue.source as? AddPetTableViewController {
            if let pet = addTableViewController.pet {
                pets.append(pet)
                
                let indexPath = IndexPath(row: pets.count-1, section: 0)
